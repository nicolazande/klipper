...
#include "serialservo_solve.h" // serialservo_solve_generate_steps
#include "pyhelper.h" // errorf
#include "stepcompress.h" // queue_append_start
#include "serialservo_compress.h" // queue_append_start
...
static int32_t
gen_steps_range(struct serialservo_kinematics *sk, struct move *m, double abs_start, double abs_end)
{
    /* simulation data */
 ...
 */
static int
check_active(struct serialservo_kinematics *sk, struct move *m)
{
 ...
 */
int32_t __visible
serialservo_solve_generate_steps(struct serialservo_kinematics *sk, double flush_time)
{
    /* save last flush */
...
double __visible
serialservo_solve_check_active(struct serialservo_kinematics *sk, double flush_time)
{
...
int32_t __visible
serialservo_solve_is_active_axis(struct serialservo_kinematics *sk, char axis)
{
    /* check axis name */
    if (axis < 'x' || axis > 'z')
    {
...
void __visible
serialservo_solve_set_trapq(struct serialservo_kinematics *sk, struct trapq *tq)
{
    sk->tq = tq;
}
...
void __visible
serialservo_solve_set_stepcompress(struct serialservo_kinematics *sk, struct stepcompress *sc, double simtime)
{
    sk->sc = sc;
    sk->simtime = simtime;
}
...
double __visible
serialservo_solve_calc_position_from_coord(struct serialservo_kinematics *sk,
                                           double x,
                                           double y,
                                           double z)
{
...
void __visible
serialservo_solve_set_position(struct serialservo_kinematics *sk,
                               double x,
                               double y,
                               double z)
{
    sk->commanded_pos = serialservo_solve_calc_position_from_coord(sk, x, y, z);
}

/** get toolhead position (last move sampled) */
double __visible
serialservo_solve_get_commanded_pos(struct serialservo_kinematics *sk)
{
