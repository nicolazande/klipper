...
#include "pvtsolve.h" // pvtsolve_generate_steps for pvt
#include "pyhelper.h" // errorf
#include "pvtcompress.h" // queue_append_start
...
gen_steps_range(struct drive_kinematics *sk, struct move *m, double abs_start, double abs_end)
{
    ...
    sk_fwk_callback kinematics_cb = sk->kinematics_cb;
    ...
    while (start + MIN_MOVE_TIME < end)
    {
         ...
         */
        pose = kinematics_cb(sk, m, start);

        /* append move to message queue */
        pvtcompress_append(sk->sc, &pose, dt);
 ...
 */
int32_t __visible
pvtsolve_generate_steps(struct drive_kinematics *sk, double flush_time)
{
    /* save last flush */
...
double __visible
pvtsolve_check_active(struct drive_kinematics *sk, double flush_time)
{
...
int32_t __visible
pvtsolve_is_active_axis(struct drive_kinematics *sk, char axis)
{
    /* check axis name */
    if (axis < 'x' || axis > 'z')
    {
...
void __visible
pvtsolve_set_trapq(struct drive_kinematics *sk, struct trapq *tq)
{
    sk->tq = tq;
}
...
double __visible
pvtsolve_calc_position_from_coord(struct drive_kinematics *sk,
                                  double x,
                                  double y,
                                  double z)
{
    ...
    m.start_pos.z = z;
    m.move_t = 0.;
    return sk->kinematics_cb(sk, &m, 0.).position;
}

/** set toolhead position */
void __visible
pvtsolve_set_position(struct drive_kinematics *sk,
                      double x,
                      double y,
                      double z)
{
    sk->commanded_pos = pvtsolve_calc_position_from_coord(sk, x, y, z);
}

/** get toolhead position (last move sampled) */
double __visible
pvtsolve_get_commanded_pos(struct drive_kinematics *sk)
{
